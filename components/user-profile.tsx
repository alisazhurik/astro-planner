...
import { ArrowLeft, User, CalendarIcon, Clock, MapPin, LogOut, Save } from "lucide-react"
...
export function UserProfile({ user, onUpdateBirthData, onBack, onLogout }: UserProfileProps) {
  const [isEditing, setIsEditing] = useState(false)
  const [name, setName] = useState(user.birthData?.name || "")
  const [birthMonth, setBirthMonth] = useState(
    user.birthData?.dateOfBirth ? (user.birthData.dateOfBirth.getMonth() + 1).toString() : "",
  )
  const [birthDay, setBirthDay] = useState(
    user.birthData?.dateOfBirth ? user.birthData.dateOfBirth.getDate().toString() : "",
  )
  const [birthYear, setBirthYear] = useState(
    user.birthData?.dateOfBirth ? user.birthData.dateOfBirth.getFullYear().toString() : "",
  )
  const [timeOfBirth, setTimeOfBirth] = useState(user.birthData?.timeOfBirth || "")
  const [placeOfBirth, setPlaceOfBirth] = useState(user.birthData?.placeOfBirth || "")
  ...
  const handleCancel = () => {
    ...
    setBirthMonth(user.birthData?.dateOfBirth ? (user.birthData.dateOfBirth.getMonth() + 1).toString() : "")
    setBirthDay(user.birthData?.dateOfBirth ? user.birthData.dateOfBirth.getDate().toString() : "")
    setBirthYear(user.birthData?.dateOfBirth ? user.birthData.dateOfBirth.getFullYear().toString() : "")
          ...
          <CardContent className="space-y-6">
            {!isEditing ? (
                  ...
                  <>
                    <div className="text-center mb-6">
                      <div className="w-20 h-20 bg-gradient-to-br from-purple-400 to-indigo-400 rounded-full flex items-center justify-center mx-auto mb-4">
                        <span className="text-3xl">{getZodiacSign(user.birthData.dateOfBirth).emoji}</span>
                      </div>
                      <h2 className="text-2xl font-bold text-gray-800">{user.birthData.name}</h2>
                      <p className="text-lg text-purple-600">{getZodiacSign(user.birthData.dateOfBirth).sign}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-gray-800">{format(user.birthData.dateOfBirth, "MMMM d, yyyy")}</p>
                      </div>
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-gray-800">{user.birthData.timeOfBirth}</p>
                      </div>
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg md:col-span-2">
                        ...
                        <p className="text-gray-800">{user.birthData.placeOfBirth}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <span className="ml-2 font-medium">{user.tasks.length}</span>
                        </div>
                        <div>
                          <span className="text-purple-600">Completed:</span>
                          <span className="ml-2 font-medium">{user.tasks.filter((t) => t.completed).length}</span>
                        </div>
                      </div>
                    </div>
                  </>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="place" className="text-gray-700 flex items-center gap-2">
